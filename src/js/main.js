...
$(document).ready(function() {
	...
	$.getJSON("../klickspel/recipes.json", function(data) {
		recipe = data.recipe;
...
function dragAndDrop() {
	...
	$('.draggableItem').draggable({
		revert: function(happyFace){
			...
			if(!happyFace){
				var normalFace = "<img src='../klickspel/img/normal.png'>";
					$('#face').html("<img src='../klickspel/img/fail.png'>");
					setTimeout(function(){
		...
		drop:function(event, ui){
			...
			for (var i=1; i < totalRecipe; i++) {
				if(recipe[i].id == userAnswer){
					...
					var normalFace = "<img src='../klickspel/img/normal.png'>";
					$('#face').html("<img src='../klickspel/img/success.png'>");
					setTimeout(function(){
						$('#face').html(normalFace);
					}, 1000);
				}
...
function parseXML() {
	...
	if (isIE == true || isEdge == true || isFirefox == true) {
	    ...
	    do {
	        ranking += '<tr>';
			ranking += '<td><img src="../klickspel/img/star-green.png" alt="greenstar">' + player[i].children[0].innerHTML + '</td>';
